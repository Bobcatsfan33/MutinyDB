...
| --- | --- | --- |
| Composed fleet wake (this repo's sim) | **p50 72 ms / p99 407 ms** wake-to-first-answer, 10,000 sleeping tenants at ~74 MB RSS under a **2 GiB cgroup ceiling** (`crates/mutinyd/evidence/m7-fleet-sim.json`) | **same-host, local filesystem**, release mode, nightly `fleet-sim-10k` CI job under `systemd-run MemoryMax=2G`; wake = engine snapshot+suffix open + membership hydration + one waking write + first standing answer; an uncapped macOS run corroborates in the same ledger |
